...
# import the csv file, set the 'sep' and 'dec' correctly
data <- read.csv(file="data_ecg.csv", header = TRUE, sep = ';', dec = ",")

# set all the variables (columns) as numeric (quotient scale) or factor (nominal scale)
...
# calculating AE and RATIO, the coefficient form the Td and Rd in database for every row
library(plyr)
library(dplyr)
data <- mutate(data, AE = abs(Rd-Td)/Td)
data <- mutate(data, RATIO = Rd/Td)

# aggregation of the coefficient for groups based on factors: participant, sound, duration, A2
# we calculate mean and sd within those groups
...
# calculating CV, the coefficient form the Rd aggregated in new database for every row
agg_data <- mutate(agg_data, CV = Rd.sd/Rd.mean)

# checking normality by function ... 
library(ggplot2)
ks.test(data$AE, "pnorm")

# ... and by eye-check, on histogram for every factor
...
### two-way ANOVA's for preview checking of the dependencies between factors
...
### bayes models for AE coefficient

# base model: no influence of factors 
m0<- brm(AE ~ (1|participant), 
         data = data, family = shifted_lognormal(link = "identity", link_sigma = "log",
                                                 link_ndt = "log"),
         chains = 2, # equal to the number of cores
         iter = 2000, # 1000x number of cores
         cores = 2,  # equal to the number of cores in your computer
         warmup  = 500, # good to be 1/4 of numbers of iterations 
         save_all_pars = TRUE) 

summary(m0)
plot(m0)
pp_check(m0) # we match the family of the model till the blue plot will mantle the black
...
plot(m1a)
pp_check(m1a) 
marginal_effects(m1a, 'duration') # to check the boxplots, potential effect
...
#we calculate WAIC for every model 
waic_m0 <- WAIC(m0)
waic_m1a <- WAIC(m1a)
waic_m1b <- WAIC(m1b)
waic_m1c <- WAIC(m1c)
...
# and compare them within to find the group of factors that explains best our data
